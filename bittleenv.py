...
class Bittle(gym.Env):
    ...
    def __init__(self,connect_GUI=False):
        ...
        p.resetDebugVisualizerCamera(cameraDistance=1.2, cameraYaw=80, cameraPitch=-10,
                                     cameraTargetPosition=[0.05, -0.15, 0.05])
        self.action_space = spaces.Box(specs[:,1], specs[:,2])
        self.observation_space = spaces.Box(np.array([-1]*3 + specs[:,1].tolist() + [-15]*3 + [-maxVelo*1.5]*12),
                                            np.array([ 1]*3 + specs[:,2].tolist() + [ 15]*3 + [ maxVelo*1.5]*12) )
        self.cnt_ = 0
        self.cnt2 = 0

        self.configs = ninepose_config
    ...
    def step(self,action):
        ...
        for s in range(40):
            ...
            p.stepSimulation() # 40/1000 = 1/25
            if self.gui:
                pass
        self.torq/= 40

        pos, velo, omega, grav, react, torq = self.get_state()
        observation = grav + pos + omega + velo
        ...
        cos_ori = grav[2] * (-1)
        ori_thres = (cos_ori > np.cos(0.2*pi))
        ori_reward = 0.667 * (0.5 * cos_ori + 0.5) ** 2 # used

        height_reward = 0.333 * np.clip( (height-0.06)/0.25, 0.0, 1.0) * ori_thres
        pose_weights = np.array([1.2, 1.5, 0.6] * 4)
        tgt_pose = np.array([0, 0.7, -1.5] * 4) * pose_weights
        pose_weighted = np.array(pos) * pose_weights
        nominal_pose_reward = 0.667 * rbf_reward(pose_weighted,tgt_pose, -0.5) * ori_thres
        standing_velo_regu = 0.333 * rbf_reward(velo, 0, -0.02) * nominal_pose_reward
        ...
        delta_pose_reward = 0.067 * rbf_reward(new_pos,old_pos,-0.5)   # used
        
        no_flip = - 10 * ( np.abs(grav[0])>np.cos(0.2*pi) )  # used

        reward = ori_reward + stand_reward + foot_contact_reward + body_contact_reward \
                 + joint_torq_regu_reward + delta_pose_reward + no_flip
        reward = reward * 0.04   # dt
        info = {'ori_reward':ori_reward, 'ori_thres':ori_thres,
                ...
                'joint_torq_regu_reward':joint_torq_regu_reward,
                'delta_pose_reward':delta_pose_reward, 'no_flip':no_flip,
                }
        if self.gui:
