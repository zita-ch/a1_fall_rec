...
env._max_episode_steps = 500+10 # for sample/estimation # 75 # rollout
env.dr = False
env.friction = 0.7
env.seed(args.seed)
...
for _ in range(episodes):
    ...
    while (episode_steps < env._max_episode_steps):
        ...
        thr_ = 0.065
        if reward > thr_:
            ratio = np.clip((reward-thr_)/(0.085-thr_),0,1)
        ...
        print(episode_steps*0.04, reward, info['joint_torq_regu_reward'],info['nominal_pose_reward'],info['no_flip'])
